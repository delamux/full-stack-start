...
export const rbacUserMiddleware =
  ...
  (
    ...
    const permissionFound = permissions.find((permission) => {
      ...
      const byPassAuth =
        permission?.bypassAuth !== undefined && permission?.bypassAuth === true;
...
function hasRoleMatch(
  ...
  return (
    roles.includes(UserRole.ALL) ||
    roles.some((role) => userRoles.includes(role))
  );
}

function hastMethodMatched(
  permission: UserPermission,
  reqMethod: Method,
): boolean {
  return (
    Array.isArray(permission.methods) &&
    permission.methods.some((method) => reqMethod === method)
  );
}
...
function hasUrlMatched(permission: UserPermission, reqPath: string): boolean {
  const isStringPath =
    ...
    (permission.path === reqPath || permission.path === ALLOW_ALL);

  const isPathIncluded =
    Array.isArray(permission.path) &&
    permission.path.some((url) => reqPath === url);
